...
func DiscoverMCEACMMirrorsFromHub(ctx context.Context, apiClient *clients.Settings) (*MCEACMMirrors, error) {
	if apiClient == nil {
	...
	for i := range idmsList.Items {
		for j := range idmsList.Items[i].Spec.ImageDigestMirrors {
			source := string(idmsList.Items[i].Spec.ImageDigestMirrors[j].Source)
			idmMirrors := idmsList.Items[i].Spec.ImageDigestMirrors[j].Mirrors

			if len(idmMirrors) == 0 {
				continue
			}

			if strings.HasPrefix(source, mceSourcePrefix) && mirrors.MCE == "" {
				mirrors.MCE = string(idmMirrors[0])
			}

			if strings.HasPrefix(source, acmSourcePrefix) && mirrors.ACM == "" {
				mirrors.ACM = string(idmMirrors[0])
			}
		}
	}
